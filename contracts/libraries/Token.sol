...
library Token {
    ...
    function safeApprove(Token.Info storage self, address spender, uint256 amount) internal {
        return SafeERC20Upgradeable(self.tokenAddress).safeApprove(spender, amount);
    }

    function safeTransfer(Token.Info storage self, address to, uint256 amount) internal {
        return SafeERC20Upgradeable(self.tokenAddress).safeTransfer(to, amount);
    }

    function safeTransferFrom(Token.Info storage self, address from, address to, uint256 amount) internal {
        return SafeERC20Upgradeable(self.tokenAddress).safeTransfer(from, to, amount);
    }

    function getBalance(Token.Info storage self) internal view {
        return SafeERC20Upgradeable(self.tokenAddress).balanceOf(address(this));
    }
