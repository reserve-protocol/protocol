...
contract Asset is IAsset {
    ...
    function minTradeSize() external view virtual returns (uint192) {
        (uint192 min, ) = _tradeSizes();
        return min;
    }

    /// @return {tok} The maximum trade size, must be at least minTradeSize
    function maxTradeSize() external view virtual returns (uint192) {
        (uint192 min, uint192 max) = _tradeSizes();
        return fixMax(min, max);
    }

    /// @return min {tok} The minimium trade size
    /// @return max {tok} The maximum trade size
    function _tradeSizes() internal view virtual returns (uint192 min, uint192 max) {
