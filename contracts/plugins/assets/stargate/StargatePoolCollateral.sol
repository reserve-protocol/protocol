 ...
 */
contract StargatePoolCollateral is FiatCollateral {
    ...
    /// @param config units to feed Asset like tok, target , chainlinkfeed
    constructor(CollateralConfig memory config, uint192 revenueHiding) FiatCollateral(config) {
        ...
        revenueShowing = FIX_ONE.minus(revenueHiding); 
        pool = IStargatePool(address(config.erc20));
    }
    ...
    function refresh() public virtual override {
        ...
        //////////////////// this is the flag for extreme cases in the bridge
        if (pool.stopSwap()){
