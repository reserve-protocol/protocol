...
/**
 * @title aCRVCollateral
 * @notice Collateral plugin for a aCRV collateral from Concentrator
 * Expected: {tok} != {ref}, {ref} == {target}, {target} != {UoA}
 */
contract aCRVCollateral is Collateral {
    ...
    function refPerTok() public view override returns (uint192) {
        IaCRV aCRV = IaCRV(address(erc20));
        uint256 totalSupply = aCRV.totalSupply();
        uint256 underlyingValue = aCRV.totalUnderlying();
    ...
    function claimRewards() external virtual override {
        ...
        IaCRV(address(erc20)).harvest(address(this), 0);
