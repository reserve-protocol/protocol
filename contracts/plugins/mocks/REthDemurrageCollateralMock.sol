...
/**
 * @title REthDemurrageCollateralMock
 * @notice Collateral plugin for rEth - a Demurrage Collateral Plugin.
 * Expected: {tok} != {ref}, {ref} == {target}, {target} != {UoA}
 */
contract REthDemurrageCollateralMock is Collateral {
    ...
    function refPerTok() public view override returns (uint192) {
        ...
        uint daysUncounted = (nowMinusLastTimestamp - subtractFactor) / (3600 * 24) ; 
        ...
        if(daysUncounted < maxDaysWORefresh){
            ...
            while(daysUncounted > 0){
                ...
                latestRefPerTok_ = latestRefPerTok_ * (FIX_ONE + 1e9*(demurrage_rate_per_second * 3600 * 24)) / FIX_ONE;
                daysUncounted -= 1;
            }
        return uint192(latestRefPerTok_);
        } else{
            // doesn't default the collateral

            return(latestRefPerTok);
        }
