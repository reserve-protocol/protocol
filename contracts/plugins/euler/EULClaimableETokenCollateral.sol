...
/**
 * @title eWstETH Collateral with EUL Claimability
...
contract EULClaimableETokenWSTETHCollateral is Collateral {
    ...
    // params that ought be updated before calling claimRewards
    //uint public claimable;
    ...
    //     bytes32[] proof;
    //  }
    // uint public latestClaimDataCount;
    // mapping(uint => ClaimData) public claimdata;
    ...
    /// getClaimData doesn't actually exist in EulDistributor.sol. 
    /// a contract that stores and has the getter function should be deployed in addition to this contract
    /// since the state variables in the collateral contract can't be read and used via delegate call from other Reserve core contracts
    /// @dev delegatecall
    function claimRewards() external virtual override {
        ...
        if (claimable != 0 && proof.length != 0) eulDistributor.claim(address(this), address(eul), claimable, proof, address(0));
