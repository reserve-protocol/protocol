...
library TradeLib {
        ...
        returns (bool notDust, TradeRequest memory req)
    {
        ...
        uint192 sellAmt = fixMin(trade.sellAmount, maxTradeSize(trade.sell));
    ...
    //   1 < req.sellAmount <= min(trade.sellAmount.toQTok(trade.sell),  TODO: should this be min?
    //                               sell.maxTradeSize().toQTok(trade.sell))
    //   req.minBuyAmount * trade.buyPrice ~=
    //       trade.sellAmount * trade.sellPrice * (1-rules.maxTradeSlippage)
    //   req.sellAmount (and req.minBuyAmount) are maximal satisfying all these conditions
    function prepareTradeToCoverDeficit(TradeInfo memory trade, TradingRules calldata rules)
        internal
        view
        returns (bool notDust, TradeRequest memory req)
    {
        ...
        uint192 maxBuyAmt = fixMax(trade.buyAmount, minTradeSize(trade.buy, rules.minTradeVolume));
        // Sell at most the offered token balance and at most the maxTradeSize()
        uint192 maxSellAmt = fixMin(trade.sellAmount, maxTradeSize(trade.sell));
        ...
        //
        uint192 requestPrice = (trade.buyPrice).div(trade.sellPrice).div(
        ...
        if (maxBuyAmt.mul(requestPrice) >= maxSellAmt) {
            sellAmt = maxSellAmt;
            buyAmt = maxSellAmt.div(requestPrice);
        } else {
            sellAmt = maxBuyAmt.mul(requestPrice);
            buyAmt = maxBuyAmt;
        }
    ...
    function isEnoughToSell(
        ...
        return
            amt.gte(minTradeSize(minTradeVolume_, asset.price()) &&
            // {qTok} = {tok} / {tok/qTok}
    ...
    function minTradeSize(uint192 minTradeVolume_, uint192 price) internal view returns (uint192) {
        // {tok} = {UoA} / {UoA/tok}
        return price == 0 ? FIX_MAX : minTradeVolume_.div(price);
    }
    ...
    function maxTradeSize(IAsset asset, uint192 price) internal view returns (uint192) {
        return price == 0 ? FIX_MAX : asset.maxTradeVolume().div(price);
    }
}
