...
library BasketLib {
    ...
    function empty(Basket storage self) internal {
        for (uint256 i = 0; i < self.tokens.length; i++) {
            self.refAmts[self.tokens[i]] = FIX_ZERO;
        }
        delete self.tokens;
        self.nonce++;
    }
    ...
    function copy(Basket storage self, Basket storage other) internal {
        empty(self);
        for (uint256 i = 0; i < other.tokens.length; i++) {
            self.tokens.push(other.tokens[i]);
            self.refAmts[other.tokens[i]] = other.refAmts[other.tokens[i]];
        }
        self.nonce++;
    }
