...
const testTokens: TestTokenParams[] = [
  ...
  //   whale: '0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2'
  // },
  // CRV
  // {
...
describe(`RToken Zapper Test V1`, () => {
  ...
  before(async () => {
    ...
    const zapRouterDeploy: ZapRouter = (await ZapRouterFactory.deploy(50)) as ZapRouter;
  ...
  async function verifyTokenInputs(inputs: TestTokenParams[]) {
    for (const input of inputs) {
      const { address, whale } = input;
      for (const targetBasket of testBaskets) {
        await verifyMint(address, whale, acquireAmount, spendAmount, targetBasket);
      }
    }
  }

  async function verifyMint(purchaseToken: string, whale: string, acquireAmount: BigNumber, spendAmount: BigNumber, targetBasket: string) {
    ...
    console.log(`Minted ${Number(ethers.utils.formatEther(rTokenBalanceAfter.toString())).toFixed(2)} ${basketName} with ${Number(ethers.utils.formatUnits(convertedSpend.toString(), decimals)).toFixed(2)} ${tokenName}`);
