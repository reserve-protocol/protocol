...
const collateralSpecificConstructorTests = () => {
  it('tokenised deposits can correctly claim rewards', async () => {
    ...
    const morphoRewards = await ethers.getContractAt(
      ...
      networkConfig[1].MORPHO_REWARDS_DISTRIBUTOR!
    )
...
export const defaultCollateralOpts: MAFiatCollateralOpts = {
  targetName: ethers.utils.formatBytes32String('USDT'),
  underlyingToken: networkConfig[1].tokens.USDT!,
  poolToken: networkConfig[1].tokens.aUSDT!,
...
const opts = {
  ...
  collateralName: 'MorphoAAVEV2FiatCollateral',
