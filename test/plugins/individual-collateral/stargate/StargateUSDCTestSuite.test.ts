...
const deployCollateralStargateMockContext = async (
  ...
  const tokDecimals = await wrapper.decimals()
...
const reduceRefPerTok = async (ctx: StargateCollateralFixtureContext) => {
  const currentExchangeRate = await ctx.collateral.refPerTok()
  await ctx.pool.setExchangeRate(currentExchangeRate.sub(fp('0.001')))
}
