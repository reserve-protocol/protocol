...
export const transferMaplePoolToken = async (
  holder: string,
  mToken: IMaplePool,
  amount: BigNumberish,
  recipient: string
...
export const getExpectedPrice = async (ctx: CollateralFixtureContext) => {
  ...
  return _targetPerRef.answer
    .mul(bn(10).pow(18 - _decimals))
    .mul(_refPerTok)
    .div(fp('1'))
}

export const increaseTargetPerRef = async (
...
export const reduceTargetPerRef = async (
  ctx: CollateralFixtureContext,
...
export const increaseRefPerTokFactory = (underlying: string, holder: string) => {
  const _increaseRefPerTok = async (ctx: CollateralFixtureContext, pctIncrease: BigNumberish) => {
    const _underlying = await ethers.getContractAt('IERC20Metadata', underlying)
    const _manager = await ethers.getContractAt(
      'IPoolManager',
      await (ctx.tok as IMaplePool).manager()
    )
...
export const reduceRefPerTokFactory = (underlying: string, holder: string, cover: number) => {
  const _reduceRefPerTok = async (ctx: CollateralFixtureContext, pctDecrease: BigNumberish) => {
    ...
    if (cover > 0)
      await mintMaplePoolToken(underlying, holder, ctx.tok.address, _amount.mul(bn(cover)), holder)
