...
import { makeWUSDCeUSD, mintLpToken, mintWrappedLpToken, resetFork } from './helpers'
...
describeFork('Aerodrome Gauge Wrapper', () => {
  ...
  beforeEach(async () => {
    ...
    ;({ token0, token1, wrapper, lpToken } = await loadFixture(makeWUSDCeUSD))
    gauge = await ethers.getContractAt('IAeroGauge', AERO_USDC_eUSD_GAUGE)
  })
  ...
  describe('Deposit', () => {
    ...
    beforeEach(async () => {
      await mintLpToken(gauge, lpToken, amount, AERO_USDC_eUSD_HOLDER, bob.address)
  ...
  describe('Rewards', () => {
    ...
    it('distributes rewards to holder', async () => {
      ...
      expect(await aero.balanceOf(don.address)).to.be.closeTo(
        expectedRewardsDon,
        point1Pct(expectedRewardsDon)
      )
    })
  })

  // TODO: Aerodrome exceptions?
})
