...
export const deployCollateral = async (opts: CollateralOpts = {}): Promise<TestICollateral> => {
  ...
  const USDeCollateralFactory: ContractFactory = await ethers.getContractFactory('USDeCollateral')
  const collateral = <TestICollateral>await USDeCollateralFactory.deploy(
    {
      erc20: opts.erc20,
...
const opts = {
  ...
  collateralName: 'USDe Collateral',
