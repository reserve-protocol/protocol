...
import { BEND } from '.././bend-weth/constants'
    ...
    describe('collateral functionality', () => {
      ...
      describe('rewards', () => {
        ...
        itClaimsRewards('claims rewards', async () => {
          ...
          const rewardToken = await ethers.getContractAt(`IBToken`, BEND )
          const balBefore = await rewardToken.balanceOf(collateral.address)
          
          await expect(collateral.claimRewards()).to.emit(collateral, 'RewardsClaimed')
          const balAfter = await rewardToken.balanceOf(collateral.address)
