...
const makeCollateralFixtureContext = (
  ...
  const makeCollateralFixtureContext = async () => {
    ...
    const realCurvePool = <CurvePoolMock>await ethers.getContractAt('CurvePoolMock', ARB_crvUSD_USDC)
    ...
    const crvUsdUSDCPool = <IConvexRewardPool> await ethers.getContractAt('IConvexRewardPool', ARB_Convex_crvUSD_USDC)
...
const collateralSpecificStatusTests = () => {
  it('Claims rewards', async () => {
    ...
    const crvUsdUSDCPool = <IConvexRewardPool> await ethers.getContractAt('IConvexRewardPool', ARB_Convex_crvUSD_USDC)
    await mintL2Pool({ wrapper: crvUsdUSDCPool } as CurveBase, amt, collateral.address, ARB_crvUSD_USDC_HOLDER)
