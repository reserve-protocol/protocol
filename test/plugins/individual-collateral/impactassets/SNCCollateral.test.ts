...
import { resetFork, mintSNC } from './helpers'
...
interface SNCCollateralFixtureContext extends CollateralFixtureContext {
  weth: WETH9
  SNC: ERC20Mock
  ISNC: ISNC
...
interface SNCCollateralFixtureContextMock extends SNCCollateralFixtureContext {
  SNCMock: SNCMock
}

interface SNCCollateralOpts extends CollateralOpts {
...
export const defaultSNCCollateralOpts: ApxEthCollateralOpts = {
  erc20: SNC,
  targetName: ethers.utils.formatBytes32String('SNC'),
...
export const deployCollateral = async (
  opts: SNCCollateralOpts = {}
): Promise<TestICollateral> => {
  opts = { ...defaultApxEthCollateralOpts, ...opts }

  const SNCCollateralFactory: ContractFactory = await ethers.getContractFactory(
    'SNCCollateral'
  )

  const collateral = <TestICollateral>await SNCCollateralFactory.deploy(
    {
      erc20: opts.erc20,
...
const makeCollateralFixtureContext = (
...
): Fixture<SNCCollateralFixtureContext> => {
  const collateralOpts = { ...defaultSNCCollateralOpts, ...opts }
  ...
  const makeCollateralFixtureContext = async () => {
    ...
    const SNC = (await ethers.getContractAt('ERC20Mock', SNC)) as ERC20Mock
    const SNC = (await ethers.getContractAt('IApxETH', SNC)) as ISNC
...
const deployCollateralSNCMockContext = async (
  opts: SNCCollateralOpts = {}
): Promise<SNCCollateralFixtureContextMock> => {
  const collateralOpts = { ...defaultSNCCollateralOpts, ...opts }
  ...
  const MockFactory = await ethers.getContractFactory('SNCMock')
  const erc20 = (await MockFactory.deploy()) as SNCMock
  const currentAPS = await (await ethers.getContractAt('SNC', SNC)).assetsPerShare()
  ...
  const weth = (await ethers.getContractAt('WETH9', WETH)) as WETH9
  const SNC = (await ethers.getContractAt('ERC20Mock', SNC)) as ERC20Mock
  const ISNC = (await ethers.getContractAt('ISNC', SNC)) as ISNC
...
const mintCollateralTo: MintCollateralFunc<ApxEthCollateralFixtureContext> = async (
  ctx: SNCCollateralFixtureContext,
  amount: BigNumberish,
  user: SignerWithAddress,
  recipient: string
) => {
  await mintSNC(ctx.SNC, user, amount, recipient)
}
...
const reduceTargetPerRef = async (
  ctx: PSNCCollateralFixtureContext,
...
const increaseRefPerTok = async (
  ...
  const currentBal = await ctx.pxETH.balanceOf(ctx.SNC.address)
  ...
  // Notify rewards
  await whileImpersonating(SNC, async (SNCSigner) => {
    await ctx.apxEth.connect(SNCSigner).notifyRewardAmount()
  })
...
const getExpectedPrice = async (ctx: SNCCollateralFixtureContext): Promise<BigNumber> => {
...
const collateralSpecificStatusTests = () => {
  it('does revenue hiding correctly', async () => {
    const { collateral, apxEthMock } = await deployCollateralSNCMockContext({
      revenueHiding: fp('0.01'),
    })

    const currentAPS = await (await ethers.getContractAt('ISNC', SNC)).assetsPerShare()
    ...
    await SNCMock.setAssetsPerShare(newAPS)
    ...
    await SNCMock.setAssetsPerShare(newAPS.sub(newAPS.div(100)))
    await collateral.refresh()
  ...
  it('enters DISABLED state when refPerTok() decreases', async () => {
    ...
    await SNCMock.setAssetsPerShare(currentAPS.sub(currentAPS.mul(10).div(100)))
