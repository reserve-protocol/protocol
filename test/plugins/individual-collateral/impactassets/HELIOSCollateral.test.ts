...
import { resetFork, mintHELIOS } from './helpers'
...
interface HELIOSCollateralFixtureContext extends CollateralFixtureContext {
  weth: WETH9
  HELIOS: ERC20Mock
  IHELIOS: IHELIOS
...
interface HELIOSCollateralFixtureContextMock extends HELIOSCollateralFixtureContext {
  HELIOSMock: HELIOSMock
}

interface HELIOSCollateralOpts extends CollateralOpts {
...
export const defaultHELIOSCollateralOpts: ApxEthCollateralOpts = {
  erc20: HELIOS,
  targetName: ethers.utils.formatBytes32String('HELIOS'),
...
export const deployCollateral = async (
  opts: HELIOSCollateralOpts = {}
): Promise<TestICollateral> => {
  opts = { ...defaultApxEthCollateralOpts, ...opts }

  const HELIOSCollateralFactory: ContractFactory = await ethers.getContractFactory(
    'HELIOSCollateral'
  )

  const collateral = <TestICollateral>await HELIOSCollateralFactory.deploy(
    {
      erc20: opts.erc20,
...
const makeCollateralFixtureContext = (
...
): Fixture<HELIOSCollateralFixtureContext> => {
  const collateralOpts = { ...defaultHELIOSCollateralOpts, ...opts }
  ...
  const makeCollateralFixtureContext = async () => {
    ...
    const HELIOS = (await ethers.getContractAt('ERC20Mock', HELIOS)) as ERC20Mock
    const HELIOS = (await ethers.getContractAt('IApxETH', HELIOS)) as IHELIOS
...
const deployCollateralHELIOSMockContext = async (
  opts: HELIOSCollateralOpts = {}
): Promise<HELIOSCollateralFixtureContextMock> => {
  const collateralOpts = { ...defaultHELIOSCollateralOpts, ...opts }
  ...
  const MockFactory = await ethers.getContractFactory('HELIOSMock')
  const erc20 = (await MockFactory.deploy()) as HELIOSMock
  const currentAPS = await (await ethers.getContractAt('HELIOS', HELIOS)).assetsPerShare()
  ...
  const weth = (await ethers.getContractAt('WETH9', WETH)) as WETH9
  const HELIOS = (await ethers.getContractAt('ERC20Mock', HELIOS)) as ERC20Mock
  const IHELIOS = (await ethers.getContractAt('IHELIOS', HELIOS)) as IHELIOS
...
const mintCollateralTo: MintCollateralFunc<ApxEthCollateralFixtureContext> = async (
  ctx: HELIOSCollateralFixtureContext,
  amount: BigNumberish,
  user: SignerWithAddress,
  recipient: string
) => {
  await mintHELIOS(ctx.HELIOS, user, amount, recipient)
}
...
const reduceTargetPerRef = async (
  ctx: PHELIOSCollateralFixtureContext,
...
const increaseRefPerTok = async (
  ...
  const currentBal = await ctx.pxETH.balanceOf(ctx.HELIOS.address)
  ...
  // Notify rewards
  await whileImpersonating(HELIOS, async (HELIOSSigner) => {
    await ctx.apxEth.connect(HELIOSSigner).notifyRewardAmount()
  })
...
const getExpectedPrice = async (ctx: HELIOSCollateralFixtureContext): Promise<BigNumber> => {
...
const collateralSpecificStatusTests = () => {
  it('does revenue hiding correctly', async () => {
    const { collateral, apxEthMock } = await deployCollateralHELIOSMockContext({
      revenueHiding: fp('0.01'),
    })

    const currentAPS = await (await ethers.getContractAt('IHELIOS', HELIOS)).assetsPerShare()
    ...
    await HELIOSMock.setAssetsPerShare(newAPS)
    ...
    await HELIOSMock.setAssetsPerShare(newAPS.sub(newAPS.div(100)))
    await collateral.refresh()
  ...
  it('enters DISABLED state when refPerTok() decreases', async () => {
    ...
    await HELIOSMock.setAssetsPerShare(currentAPS.sub(currentAPS.mul(10).div(100)))
