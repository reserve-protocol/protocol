...
import { resetFork, mintPOWR } from './helpers'
...
interface POWRCollateralFixtureContext extends CollateralFixtureContext {
  weth: WETH9
  POWR: ERC20Mock
  IPOWR: IPOWR
...
interface POWRCollateralFixtureContextMock extends POWRCollateralFixtureContext {
  POWRMock: POWRMock
}

interface POWRCollateralOpts extends CollateralOpts {
...
export const defaultPOWRCollateralOpts: ApxEthCollateralOpts = {
  erc20: POWR,
  targetName: ethers.utils.formatBytes32String('POWR'),
...
export const deployCollateral = async (
  opts: POWRCollateralOpts = {}
): Promise<TestICollateral> => {
  opts = { ...defaultApxEthCollateralOpts, ...opts }

  const POWRCollateralFactory: ContractFactory = await ethers.getContractFactory(
    'POWRCollateral'
  )

  const collateral = <TestICollateral>await POWRCollateralFactory.deploy(
    {
      erc20: opts.erc20,
...
const makeCollateralFixtureContext = (
...
): Fixture<POWRCollateralFixtureContext> => {
  const collateralOpts = { ...defaultPOWRCollateralOpts, ...opts }
  ...
  const makeCollateralFixtureContext = async () => {
    ...
    const POWR = (await ethers.getContractAt('ERC20Mock', POWR)) as ERC20Mock
    const POWR = (await ethers.getContractAt('IApxETH', POWR)) as IPOWR
...
const deployCollateralPOWRMockContext = async (
  opts: POWRCollateralOpts = {}
): Promise<POWRCollateralFixtureContextMock> => {
  const collateralOpts = { ...defaultPOWRCollateralOpts, ...opts }
  ...
  const MockFactory = await ethers.getContractFactory('POWRMock')
  const erc20 = (await MockFactory.deploy()) as POWRMock
  const currentAPS = await (await ethers.getContractAt('POWR', POWR)).assetsPerShare()
  ...
  const weth = (await ethers.getContractAt('WETH9', WETH)) as WETH9
  const POWR = (await ethers.getContractAt('ERC20Mock', POWR)) as ERC20Mock
  const IPOWR = (await ethers.getContractAt('IPOWR', POWR)) as IPOWR
...
const mintCollateralTo: MintCollateralFunc<ApxEthCollateralFixtureContext> = async (
  ctx: POWRCollateralFixtureContext,
  amount: BigNumberish,
  user: SignerWithAddress,
  recipient: string
) => {
  await mintPOWR(ctx.POWR, user, amount, recipient)
}
...
const reduceTargetPerRef = async (
  ctx: PPOWRCollateralFixtureContext,
...
const increaseRefPerTok = async (
  ...
  const currentBal = await ctx.pxETH.balanceOf(ctx.POWR.address)
  ...
  // Notify rewards
  await whileImpersonating(POWR, async (POWRSigner) => {
    await ctx.apxEth.connect(POWRSigner).notifyRewardAmount()
  })
...
const getExpectedPrice = async (ctx: POWRCollateralFixtureContext): Promise<BigNumber> => {
...
const collateralSpecificStatusTests = () => {
  it('does revenue hiding correctly', async () => {
    const { collateral, apxEthMock } = await deployCollateralPOWRMockContext({
      revenueHiding: fp('0.01'),
    })

    const currentAPS = await (await ethers.getContractAt('IPOWR', POWR)).assetsPerShare()
    ...
    await POWRMock.setAssetsPerShare(newAPS)
    ...
    await POWRMock.setAssetsPerShare(newAPS.sub(newAPS.div(100)))
    await collateral.refresh()
  ...
  it('enters DISABLED state when refPerTok() decreases', async () => {
    ...
    await POWRMock.setAssetsPerShare(currentAPS.sub(currentAPS.mul(10).div(100)))
