...
import { resetFork, mintGTC } from './helpers'
...
interface GTCCollateralFixtureContext extends CollateralFixtureContext {
  weth: WETH9
  GTC: ERC20Mock
  IGTC: IGTC
...
interface GTCCollateralFixtureContextMock extends GTCCollateralFixtureContext {
  GTCMock: GTCMock
}

interface GTCCollateralOpts extends CollateralOpts {
...
export const defaultGTCCollateralOpts: ApxEthCollateralOpts = {
  erc20: GTC,
  targetName: ethers.utils.formatBytes32String('GTC'),
...
export const deployCollateral = async (
  opts: GTCCollateralOpts = {}
): Promise<TestICollateral> => {
  opts = { ...defaultApxEthCollateralOpts, ...opts }

  const GTCCollateralFactory: ContractFactory = await ethers.getContractFactory(
    'GTCCollateral'
  )

  const collateral = <TestICollateral>await GTCCollateralFactory.deploy(
    {
      erc20: opts.erc20,
...
const makeCollateralFixtureContext = (
...
): Fixture<GTCCollateralFixtureContext> => {
  const collateralOpts = { ...defaultGTCCollateralOpts, ...opts }
  ...
  const makeCollateralFixtureContext = async () => {
    ...
    const GTC = (await ethers.getContractAt('ERC20Mock', GTC)) as ERC20Mock
    const GTC = (await ethers.getContractAt('IApxETH', GTC)) as IGTC
...
const deployCollateralGTCMockContext = async (
  opts: GTCCollateralOpts = {}
): Promise<GTCCollateralFixtureContextMock> => {
  const collateralOpts = { ...defaultGTCCollateralOpts, ...opts }
  ...
  const MockFactory = await ethers.getContractFactory('GTCMock')
  const erc20 = (await MockFactory.deploy()) as GTCMock
  const currentAPS = await (await ethers.getContractAt('GTC', GTC)).assetsPerShare()
  ...
  const weth = (await ethers.getContractAt('WETH9', WETH)) as WETH9
  const GTC = (await ethers.getContractAt('ERC20Mock', GTC)) as ERC20Mock
  const IGTC = (await ethers.getContractAt('IGTC', GTC)) as IGTC
...
const mintCollateralTo: MintCollateralFunc<ApxEthCollateralFixtureContext> = async (
  ctx: GTCCollateralFixtureContext,
  amount: BigNumberish,
  user: SignerWithAddress,
  recipient: string
) => {
  await mintGTC(ctx.GTC, user, amount, recipient)
}
...
const reduceTargetPerRef = async (
  ctx: PGTCCollateralFixtureContext,
...
const increaseRefPerTok = async (
  ...
  const currentBal = await ctx.pxETH.balanceOf(ctx.GTC.address)
  ...
  // Notify rewards
  await whileImpersonating(GTC, async (GTCSigner) => {
    await ctx.apxEth.connect(GTCSigner).notifyRewardAmount()
  })
...
const getExpectedPrice = async (ctx: GTCCollateralFixtureContext): Promise<BigNumber> => {
...
const collateralSpecificStatusTests = () => {
  it('does revenue hiding correctly', async () => {
    const { collateral, apxEthMock } = await deployCollateralGTCMockContext({
      revenueHiding: fp('0.01'),
    })

    const currentAPS = await (await ethers.getContractAt('IGTC', GTC)).assetsPerShare()
    ...
    await GTCMock.setAssetsPerShare(newAPS)
    ...
    await GTCMock.setAssetsPerShare(newAPS.sub(newAPS.div(100)))
    await collateral.refresh()
  ...
  it('enters DISABLED state when refPerTok() decreases', async () => {
    ...
    await GTCMock.setAssetsPerShare(currentAPS.sub(currentAPS.mul(10).div(100)))
