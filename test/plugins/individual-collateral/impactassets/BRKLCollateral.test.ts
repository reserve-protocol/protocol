...
import { resetFork, mintBRKL } from './helpers'
...
interface BRKLCollateralFixtureContext extends CollateralFixtureContext {
  weth: WETH9
  BRKL: ERC20Mock
  IBRKL: IBRKL
...
interface BRKLCollateralFixtureContextMock extends BRKLCollateralFixtureContext {
  BRKLMock: BRKLMock
}

interface BRKLCollateralOpts extends CollateralOpts {
...
export const defaultBRKLCollateralOpts: ApxEthCollateralOpts = {
  erc20: BRKL,
  targetName: ethers.utils.formatBytes32String('BRKL'),
...
export const deployCollateral = async (
  opts: BRKLCollateralOpts = {}
): Promise<TestICollateral> => {
  opts = { ...defaultApxEthCollateralOpts, ...opts }

  const BRKLCollateralFactory: ContractFactory = await ethers.getContractFactory(
    'BRKLCollateral'
  )

  const collateral = <TestICollateral>await BRKLCollateralFactory.deploy(
    {
      erc20: opts.erc20,
...
const makeCollateralFixtureContext = (
...
): Fixture<BRKLCollateralFixtureContext> => {
  const collateralOpts = { ...defaultBRKLCollateralOpts, ...opts }
  ...
  const makeCollateralFixtureContext = async () => {
    ...
    const BRKL = (await ethers.getContractAt('ERC20Mock', BRKL)) as ERC20Mock
    const BRKL = (await ethers.getContractAt('IApxETH', BRKL)) as IBRKL
...
const deployCollateralBRKLMockContext = async (
  opts: BRKLCollateralOpts = {}
): Promise<BRKLCollateralFixtureContextMock> => {
  const collateralOpts = { ...defaultBRKLCollateralOpts, ...opts }
  ...
  const MockFactory = await ethers.getContractFactory('BRKLMock')
  const erc20 = (await MockFactory.deploy()) as BRKLMock
  const currentAPS = await (await ethers.getContractAt('BRKL', BRKL)).assetsPerShare()
  ...
  const weth = (await ethers.getContractAt('WETH9', WETH)) as WETH9
  const BRKL = (await ethers.getContractAt('ERC20Mock', BRKL)) as ERC20Mock
  const IBRKL = (await ethers.getContractAt('IBRKL', BRKL)) as IBRKL
...
const mintCollateralTo: MintCollateralFunc<ApxEthCollateralFixtureContext> = async (
  ctx: BRKLCollateralFixtureContext,
  amount: BigNumberish,
  user: SignerWithAddress,
  recipient: string
) => {
  await mintBRKL(ctx.BRKL, user, amount, recipient)
}
...
const reduceTargetPerRef = async (
  ctx: PBRKLCollateralFixtureContext,
...
const increaseRefPerTok = async (
  ...
  const currentBal = await ctx.pxETH.balanceOf(ctx.BRKL.address)
  ...
  // Notify rewards
  await whileImpersonating(BRKL, async (BRKLSigner) => {
    await ctx.apxEth.connect(BRKLSigner).notifyRewardAmount()
  })
...
const getExpectedPrice = async (ctx: BRKLCollateralFixtureContext): Promise<BigNumber> => {
...
const collateralSpecificStatusTests = () => {
  it('does revenue hiding correctly', async () => {
    const { collateral, apxEthMock } = await deployCollateralBRKLMockContext({
      revenueHiding: fp('0.01'),
    })

    const currentAPS = await (await ethers.getContractAt('IBRKL', BRKL)).assetsPerShare()
    ...
    await BRKLMock.setAssetsPerShare(newAPS)
    ...
    await BRKLMock.setAssetsPerShare(newAPS.sub(newAPS.div(100)))
    await collateral.refresh()
  ...
  it('enters DISABLED state when refPerTok() decreases', async () => {
    ...
    await BRKLMock.setAssetsPerShare(currentAPS.sub(currentAPS.mul(10).div(100)))
