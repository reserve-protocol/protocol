...
import { resetFork, mintGIV } from './helpers'
...
interface GIVCollateralFixtureContext extends CollateralFixtureContext {
  weth: WETH9
  GIV: ERC20Mock
  IGIV: IGIV
...
interface GIVCollateralFixtureContextMock extends GIVCollateralFixtureContext {
  GIVMock: GIVMock
}

interface GIVCollateralOpts extends CollateralOpts {
...
export const defaultGIVCollateralOpts: ApxEthCollateralOpts = {
  erc20: GIV,
  targetName: ethers.utils.formatBytes32String('GIV'),
...
export const deployCollateral = async (
  opts: GIVCollateralOpts = {}
): Promise<TestICollateral> => {
  opts = { ...defaultApxEthCollateralOpts, ...opts }

  const GIVCollateralFactory: ContractFactory = await ethers.getContractFactory(
    'GIVCollateral'
  )

  const collateral = <TestICollateral>await GIVCollateralFactory.deploy(
    {
      erc20: opts.erc20,
...
const makeCollateralFixtureContext = (
...
): Fixture<GIVCollateralFixtureContext> => {
  const collateralOpts = { ...defaultGIVCollateralOpts, ...opts }
  ...
  const makeCollateralFixtureContext = async () => {
    ...
    const GIV = (await ethers.getContractAt('ERC20Mock', GIV)) as ERC20Mock
    const GIV = (await ethers.getContractAt('IApxETH', GIV)) as IGIV
...
const deployCollateralGIVMockContext = async (
  opts: GIVCollateralOpts = {}
): Promise<GIVCollateralFixtureContextMock> => {
  const collateralOpts = { ...defaultGIVCollateralOpts, ...opts }
  ...
  const MockFactory = await ethers.getContractFactory('GIVMock')
  const erc20 = (await MockFactory.deploy()) as GIVMock
  const currentAPS = await (await ethers.getContractAt('GIV', GIV)).assetsPerShare()
  ...
  const weth = (await ethers.getContractAt('WETH9', WETH)) as WETH9
  const GIV = (await ethers.getContractAt('ERC20Mock', GIV)) as ERC20Mock
  const IGIV = (await ethers.getContractAt('IGIV', GIV)) as IGIV
...
const mintCollateralTo: MintCollateralFunc<ApxEthCollateralFixtureContext> = async (
  ctx: GIVCollateralFixtureContext,
  amount: BigNumberish,
  user: SignerWithAddress,
  recipient: string
) => {
  await mintGIV(ctx.GIV, user, amount, recipient)
}
...
const reduceTargetPerRef = async (
  ctx: PGIVCollateralFixtureContext,
...
const increaseRefPerTok = async (
  ...
  const currentBal = await ctx.pxETH.balanceOf(ctx.GIV.address)
  ...
  // Notify rewards
  await whileImpersonating(GIV, async (GIVSigner) => {
    await ctx.apxEth.connect(GIVSigner).notifyRewardAmount()
  })
...
const getExpectedPrice = async (ctx: GIVCollateralFixtureContext): Promise<BigNumber> => {
...
const collateralSpecificStatusTests = () => {
  it('does revenue hiding correctly', async () => {
    const { collateral, apxEthMock } = await deployCollateralGIVMockContext({
      revenueHiding: fp('0.01'),
    })

    const currentAPS = await (await ethers.getContractAt('IGIV', GIV)).assetsPerShare()
    ...
    await GIVMock.setAssetsPerShare(newAPS)
    ...
    await GIVMock.setAssetsPerShare(newAPS.sub(newAPS.div(100)))
    await collateral.refresh()
  ...
  it('enters DISABLED state when refPerTok() decreases', async () => {
    ...
    await GIVMock.setAssetsPerShare(currentAPS.sub(currentAPS.mul(10).div(100)))
