...
describe('InsurancePool contract', () => {
  ...
  describe('Deposits/Staking', () => {
    ...
    context('With stakes/deposits created', async () => {
      ...
      beforeEach(async () => {
        // Set stakingDelay
        await rToken.connect(owner).updateConfig({ ...config, stakingDepositDelay, insurancePool: iPool.address })
  ...
  describe('Withdrawals/Unstaking', () => {
    ...
    it('Should not allow to unstake amount = 0', async () => {
      ...
      // Unstake
      await expect(iPool.connect(addr1).unstake(zero)).to.be.revertedWith('CannotWithdrawZero()')
    ...
    it('Should not allow to unstake if not enough balance', async () => {
      ...
      await expect(iPool.connect(other).unstake(amount)).to.be.revertedWith('NotEnoughBalance()')
  ...
  describe('Revenues', () => {
    ...
    it('Should not allow to register Revenue if caller is not Rtoken', async () => {
      ...
      await expect(iPool.connect(owner).makeInsurancePayment(amount)).to.be.revertedWith('OnlyRToken()')
    ...
    it('Should allow to register Revenue if caller is Rtoken', async () => {
      ...
      await expect(rToken.makeInsurancePayment(amount)).to.emit(iPool, 'RevenueEventSaved').withArgs(0, amount)
    ...
    it('Should allow to register multiple Revenues', async () => {
      ...
      await expect(rToken.makeInsurancePayment(amount1)).to.emit(iPool, 'RevenueEventSaved').withArgs(0, amount1)
      ...
      await expect(rToken.makeInsurancePayment(amount2)).to.emit(iPool, 'RevenueEventSaved').withArgs(1, amount2)
    ...
    context('With revenues registered', async () => {
      ...
      beforeEach(async () => {
        ...
        // Register Revenue
        await expect(rToken.makeInsurancePayment(revAmount)).to.emit(iPool, 'RevenueEventSaved').withArgs(0, revAmount)
      ...
      it('Should allow to catchup in multiple small transactions', async () => {
        ...
        await expect(iPool.catchup(addr1.address, 2)).to.emit(iPool, 'AccountPendingUpdate').withArgs(addr1.address)
        ...
        await expect(iPool.catchup(addr1.address, 1)).to.emit(iPool, 'AccountPendingUpdate').withArgs(addr1.address)
        ...
        await expect(iPool.catchup(addr1.address, 2)).to.not.emit(iPool, 'AccountPendingUpdate')
      ...
      it('Should not send funds if no revenues to claim', async () => {
        await expect(iPool.connect(addr2).claimRevenue()).to.not.emit(iPool, 'RevenueClaimed')
