...
describe('RSR contract', function () {
  ...
  beforeEach(async function () {
    ...
    token = <RSR>await RSR.connect(owner).deploy(prevToken.address, slowWallet.address, multisigWallet.address)
  })
  ...
  describe('Balances and Transfers - After Pausing Previous RSR', function () {
    ...
    it('Should not transfer tokens to self', async function () {
      ...
      await expect(token.connect(owner).transfer(token.address, amount)).to.be.revertedWith('TransferToContractAddress')
    })
