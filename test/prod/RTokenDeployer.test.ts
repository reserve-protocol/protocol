...
describe('RTokenDeployer contract', function () {
  ...
  beforeEach(async function () {
    ...
    factory = <RTokenDeployer>(
      await RTokenFactory.connect(owner).deploy(rTokenImplementation.address, iPoolImplementation.address)
    )
  })
  ...
  describe('Creating RTokens', function () {
    beforeEach(async function () {
      ...
      // Create a new RToken
      const receipt = await (
        await factory.deploy(newOwner.address, 'RToken Test', 'RTKN', config, basketTokens, rsrTokenInfo)
      ).wait()
  ...
  describe('Upgradeability', function () {
    beforeEach(async function () {
      ...
      iPoolInstance = <InsurancePoolMock>await ethers.getContractAt('InsurancePoolMock', iPoolAddress)
    })
    ...
    describe('RToken Upgradeability', function () {
      ...
      it('Should not allow upgrades to RToken if not Owner', async function () {
        ...
        await expect(rTokenInstance.connect(other).upgradeTo(rTokenV2Implementation.address)).to.be.revertedWith(
    ...
    describe('InsurancePool Upgradeability', function () {
      ...
      it('Should not allow upgrades to InsurancePool if not Owner', async function () {
        ...
        // Try to update implementation
        await expect(iPoolInstance.connect(other).upgradeTo(iPoolV2Implementation.address)).to.be.revertedWith(
          'Ownable: caller is not the owner'
        )
      })
    })
