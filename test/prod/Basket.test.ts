...
describe('Basket library', () => {
  ...
  const createToken = async (name: string, symbol: string, genesisQuantity: BigNumberish): Promise<ITokenInfo> => {
  ...
  describe('Tokens', () => {
    ...
    it('Should return weights correctly', async () => {
      ...
      await expect(caller.weight(BN_SCALE_FACTOR, 2)).to.be.revertedWith('InvalidTokenIndex()')
    })
    ...
    context('With collateral balance', async () => {
      ...
      beforeEach(async function () {
        ...
        token1 = await (<Promise<ERC20Mock>>ethers.getContractAt('ERC20Mock', tokenInfo0.tokenAddress))
        await token1.mint(caller.address, fp_500)

        token2 = await (<Promise<ERC20Mock>>ethers.getContractAt('ERC20Mock', tokenInfo1.tokenAddress))
      ...
      it('Should not return collateralized indexes if situation is stable', async () => {
        ...
        const indexes = await caller.mostUndercollateralizedAndMostOverCollateralized(BN_SCALE_FACTOR, 18, totalSupply)
