...
const scenarioSpecificTests = () => {
  ...
  */
  ...
  it('batchRebalancingProperties holds after issueTo, unregisterAsset, refreshBasket and stake', async () => {
    // Echidna sequence (rebalancing10): issueTo(12,0) → setBackingManagerMinTradeVolume(2) →
    // unregisterAsset(0) → refreshBasket → stake(16)
    await advanceTime(259689)
    await advanceBlocks(1)
    await scenario.connect(alice).issueTo(12, 0)
    await scenario.connect(alice).setBackingManagerMinTradeVolume(2)
    await scenario.connect(alice).unregisterAsset(0)
    await scenario.connect(alice).refreshBasket()
    await advanceTime(265215)
    await advanceBlocks(1)
    await scenario.connect(alice).stake(16)

    expect(await scenario.callStatic.echidna_batchRebalancingProperties()).to.be.true
  })

  it('batchRebalancingProperties holds after setDistribution and pushRedeemableBasketNonce', async () => {
    // Echidna sequence (rebalancing11): setDistribution → issueTo(4474465,10) → setBackingManagerMinTradeVolume(0) →
    // unregisterAsset(0) → setDistribution → refreshBasket → pushRedeemableBasketNonce(0)
    await scenario.connect(alice).setDistribution(7, 1170, 0)
    await advanceTime(260451)
    await advanceBlocks(1)
    await scenario.connect(alice).issueTo(4474465, 10)
    await scenario.connect(alice).setBackingManagerMinTradeVolume(0)
    await scenario.connect(alice).unregisterAsset(0)
    await scenario.connect(alice).setDistribution(bn('17565351216043000451698338070044768'), 0, 0)
    await scenario.connect(alice).refreshBasket()
    await advanceTime(267273)
    await advanceBlocks(1)
    await scenario.connect(alice).pushRedeemableBasketNonce(0)
  ...
  it('batchRebalancingProperties does not revert after issueTo(1) and updatePrice', async () => {
    // Echidna sequence (rebalancing14): issueTo(1,0) → updatePrice → refreshBasket → updatePrice
    await warmup()
    await advanceTime(259903)
    await advanceBlocks(1)
    await scenario.connect(alice).issueTo(1, 0)
    await scenario.connect(alice).updatePrice(
      bn('2069620613063539078623794909072364140908530124402581410068459240149022'),
      0,
      608827,
      bn('42513599708914548050505987463472954998'),
      bn('248218245977572863551219678602755372476643')
    )
    await scenario.connect(alice).refreshBasket()
    ...
    await scenario.connect(alice).updatePrice(
      28335,
      bn('18261263094308454983698383596386151694651'),
      0,
      bn('34685083978808251838494002778604357455449'),
      bn('15867639247735914061957926659633209131910')
    )
