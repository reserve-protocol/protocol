...
describe("RSR contract", function () {
    ...
    let slowWallet: SignerWithAddress
    let multisigWallet: SignerWithAddress
    let prevToken: ReserveRightsTokenMock
    let token: RSR
    ...
    beforeEach(async function () {
        ...
        prevToken = <ReserveRightsTokenMock>await PrevRSR.deploy("Reserve Rights", "RSR")
        ...
        token = <RSR>(
            await RSR.connect(owner).deploy(
                prevToken.address,
                slowWallet.address,
    ...
    describe("Balances and Transfers - Before Pausing Previous RSR", function () {
        it("Should return balances from previous RSR if not crossed", async function () {
            ...
            expect(await token.balanceOf(owner.address)).to.equal(
                await prevToken.balanceOf(owner.address)
            )
            expect(await token.balanceOf(addr1.address)).to.equal(
                await prevToken.balanceOf(addr1.address)
            )
            expect(await token.balanceOf(addr2.address)).to.equal(
                await prevToken.balanceOf(addr2.address)
            )
        ...
        it("Should not transfer tokens between accounts if Previous RSR is not paused", async function () {
            ...
            await expect(token.connect(owner).transfer(addr1.address, amount)).to.be.revertedWith(
    ...
    describe("Balances and Transfers - After Pausing Previous RSR", function () {
        ...
        it("Should not transfer tokens to self", async function () {
            ...
            await expect(token.connect(owner).transfer(token.address, amount)).to.be.revertedWith(
        ...
        it("Should add slowWallet balance when crossing multisig", async function () {
            ...
            expect(await token.balanceOf(addr1.address)).to.equal(addr1BalancePrev.add(amount))
            expect(await token.balanceOf(multisigWallet.address)).to.equal(
