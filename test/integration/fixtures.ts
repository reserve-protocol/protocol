...
async function compAaveFixture(): Promise<COMPAAVEFixture> {
  ...
  // Get AAVE token
  const aaveToken: ERC20Mock = <ERC20Mock>await ethers.getContractAt('ERC20Mock', AAVE_ADDRESS)
...
async function collateralFixture(
  ...
  const makeVanillaCollateral = async (tokenAddr: string): Promise<[ERC20Mock, Collateral]> => {
    const erc20: ERC20Mock = <ERC20Mock>await ethers.getContractAt('ERC20Mock', tokenAddr)
    return [
      erc20,
      <Collateral>(
  ...
  const makeSixDecimalCollateral = async (tokenAddr: string): Promise<[USDCMock, Collateral]> => {
    const erc20: USDCMock = <USDCMock>await ethers.getContractAt('USDCMock', tokenAddr)
    return [
      erc20,
      <Collateral>(
        await AaveCollateralFactory.deploy(
  ...
  const makeCTokenCollateral = async (
    ...
    const erc20: CTokenMock = <CTokenMock>await ethers.getContractAt('CTokenMock', tokenAddress)
    return [
      erc20,
      <CTokenFiatCollateral>(
  ...
  const basketsNeededAmts = [fp('0.5'), /*fp('0.25'),*/ fp('0.5')]
