...
const holderRWBTC = '0x8913eab16a302de3e498bba39940e7a55c0b9325'
...
describeFork(
  `DMVaultTokenNonFiatCollateral (Theta BTC Vault) - Mainnet Forking P${IMPLEMENTATION}`,
  function () {
    ...
    // Tokens/Assets
    let wbtc: ERC20Mock
    let rWbtc: VaultTokenMock
    let rWbtcCollateral: DMVaultTokenNonFiatCollateral
    let rsr: ERC20Mock
    let rsrAsset: Asset
    ...
    beforeEach(async () => {
      ...
      // Get required contracts for rWBTC
      // wBTC token
      wbtc = <ERC20Mock>(
        await ethers.getContractAt('ERC20Mock', networkConfig[chainId].tokens.WBTC || '')
      )
      // rWBTC token
      rWbtc = <VaultTokenMock>(
        await ethers.getContractAt('VaultTokenMock', networkConfig[chainId].tokens.rWBTC || '')
      )

      // Deploy rWBTC collateral plugin
      ...
      rWbtcCollateral = <DMVaultTokenNonFiatCollateral>(
        await VaultTokenCollateralFactory.deploy(
          rWbtc.address,
          config.rTokenMaxTradeVolume,
          fp('1'),
          ethers.utils.formatBytes32String('DM100T-WBTC-C'),
          delayUntilDefault,
      ...
      // rWBTC
      initialBal = bn('100e8')
      await whileImpersonating(holderRWBTC, async (rWbtcSigner) => {
        await rWbtc.connect(rWbtcSigner).transfer(addr1.address, initialBal)
      })
      ...
      const rTokenSetup: IRTokenSetup = {
        assets: [],
        primaryBasket: [rWbtcCollateral.address],
        weights: [fp('1')],
        backups: [],
    ...
    describe('Deployment', () => {
      ...
      it('Should setup RToken, Assets, and Collateral correctly', async () => {
        ...
        // rWBTC (DMVaultTokenNonFiatCollateral)
        expect(await rWbtcCollateral.isCollateral()).to.equal(true)
        expect(await rWbtcCollateral.erc20()).to.equal(rWbtc.address)
        expect(await rWbtc.decimals()).to.equal(await wbtc.decimals())
        expect(await rWbtcCollateral.targetName()).to.equal(
          ethers.utils.formatBytes32String('DM100T-WBTC-C')
        )
        expect(await rWbtcCollateral.refPerTok()).to.be.closeTo(fp('1'), fp('0.01'))
        expect(await rWbtcCollateral.targetPerRef()).to.equal(fp('1'))
        expect(await rWbtcCollateral.strictPrice()).to.be.closeTo(fp('32000'), fp('200'))

        // Check claim data
        await expect(rWbtcCollateral.claimRewards()).to.not.emit(rWbtcCollateral, 'RewardsClaimed')
        expect(await rWbtcCollateral.maxTradeVolume()).to.equal(config.rTokenMaxTradeVolume)
      ...
      it('Should register ERC20s and Assets/Collateral correctly', async () => {
        ...
        expect(ERC20s[2]).to.equal(rWbtc.address)
        expect(ERC20s.length).to.eql(3)

        // Assets
        expect(await assetRegistry.toAsset(ERC20s[0])).to.equal(rTokenAsset.address)
        expect(await assetRegistry.toAsset(ERC20s[1])).to.equal(rsrAsset.address)
        expect(await assetRegistry.toAsset(ERC20s[2])).to.equal(rWbtcCollateral.address)

        // Collaterals
        expect(await assetRegistry.toColl(ERC20s[2])).to.equal(rWbtcCollateral.address)
      })
      ...
      it('Should register Basket correctly', async () => {
        ...
        expect(backing[0]).to.equal(rWbtc.address)
      ...
      it('Should validate constructor arguments correctly', async () => {
        ...
        await expect(
          VaultTokenCollateralFactory.deploy(
            ...
            config.rTokenMaxTradeVolume,
            fp('1'),
            ethers.utils.formatBytes32String('DM10000T-WBTC-C'),
            delayUntilDefault,
    ...
    describe('Issuance/Appreciation/Redemption', () => {
      ...
      it('Should issue, redeem, and handle appreciation rates correctly', async () => {
        ...
        await rWbtc.connect(addr1).approve(rToken.address, toBNDecimals(issueAmount, 8))
        ...
        const balanceAddr1RWbtc: BigNumber = await rWbtc.balanceOf(addr1.address)

        // Check rates and prices
        const rWbtcPrice1: BigNumber = await rWbtcCollateral.strictPrice()
        const rWbtcRefPerTok1: BigNumber = await rWbtcCollateral.refPerTok()

        expect(rWbtcPrice1).to.be.closeTo(fp('32000'), fp('200'))
        expect(rWbtcRefPerTok1).to.be.closeTo(fp('1'), fp('0.01'))
        ...
        // Refresh rToken manually (required)
        await rWbtcCollateral.refresh()
        expect(await rWbtcCollateral.status()).to.equal(CollateralStatus.SOUND)

        // Check rates - Have changed, slight increase
        const rWbtcRefPerTok2: BigNumber = await rWbtcCollateral.refPerTok()

        // Check rates increase
        expect(rWbtcRefPerTok2).to.be.gt(rWbtcRefPerTok1)

        // Still close to the original values
        expect(rWbtcRefPerTok2).to.be.closeTo(fp('1'), fp('0.01'))
        ...
        // Refresh theta token manually (required)
        await rWbtcCollateral.refresh()
        expect(await rWbtcCollateral.status()).to.equal(CollateralStatus.SOUND)

        // Check rates - Have changed significantly
        const rWbtcRefPerTok3: BigNumber = await rWbtcCollateral.refPerTok()

        // Check rates increase
        expect(rWbtcRefPerTok3).to.be.gt(rWbtcRefPerTok2)

        // Need to adjust ranges
        expect(rWbtcRefPerTok3).to.be.closeTo(fp('1.01'), fp('0.01'))
        ...
        // Check balances - Fewer theta vault tokens should have been sent to the user
        const newBalanceAddr1RWbtc: BigNumber = await rWbtc.balanceOf(addr1.address)

        // Check received tokens represent ~10K in value at current prices
        expect(newBalanceAddr1RWbtc.sub(balanceAddr1RWbtc)).to.be.closeTo(
          bn('0.99e8'),
          bn('0.01e8')
        )

        // Check remainders in Backing Manager
        expect(await rWbtc.balanceOf(backingManager.address)).to.be.closeTo(
          bn('0.01e8'),
          bn('0.0010e8')
        )
    ...
    describe('Price Handling', () => {
      it('Should handle invalid/stale Price', async () => {
        ...
        await expect(rWbtcCollateral.strictPrice()).to.be.revertedWith('StalePrice()')

        // Fallback price is returned
        const [isFallback, price] = await rWbtcCollateral.price(true)
        expect(isFallback).to.equal(true)
        expect(price).to.equal(fp('1'))

        // Refresh should mark status IFFY
        await rWbtcCollateral.refresh()
        expect(await rWbtcCollateral.status()).to.equal(CollateralStatus.IFFY)
      })
    })
      ...
      it('Updates status in case of soft default', async () => {
        ...
        const newYVWbtcCollateral: DMVaultTokenNonFiatCollateral = <DMVaultTokenNonFiatCollateral>(
          await (
            ...
            await rWbtcCollateral.erc20(),
            await rWbtcCollateral.maxTradeVolume(),
            fp('1'),
            await rWbtcCollateral.targetName(),
            await rWbtcCollateral.delayUntilDefault(),
            await rWbtcCollateral.ratePerPeriod(),
            networkConfig[chainId].chainlinkFeeds.BTC as string,
            mockChainlinkFeed.address,
            await rWbtcCollateral.oracleTimeout(),
            await rWbtcCollateral.defaultThreshold()
          )
        )
