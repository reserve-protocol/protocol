...
describeFork(`CTokenFiatCollateral - Mainnet Forking P${IMPLEMENTATION}`, function () {
  ...
  beforeEach(async () => {
    ...
    //curvePoolMock
    curvePoolMock = (await ConvexCurveFixutre()).curvePoolMock

    // Get required contracts for convex/curve stable pool collateral
    // COMP token
    curveLpToken = <ERC20Mock>(
      await ethers.getContractAt('ERC20Mock', networkConfig[chainId].tokens.BUSD_3CRV || '')
    )
    ...
    busd = <ERC20Mock>(
      await ethers.getContractAt('ERC20Mock', networkConfig[chainId].tokens.BUSD || '')
    )
    ...
    convexAsset = <Asset>(
      await (
        ...
        fp('1'),
        networkConfig[chainId].chainlinkFeeds.BUSD_3CRV || '',
    ...
    // Deploy CurveStablePool LP collateral plugin
    ...
    const collateralConfig: ConvexConstructorConfig = {
      ...
      rewardERC20_: curveLpToken.address,
    ...
    await whileImpersonating(holderCurveLPToken, async (curveLpTokenSigner) => {
      await curveLpToken
        ...
        .transfer(addr1.address, toBNDecimals(initialBal, 6).mul(1000))
    })
    ...
    const rTokenSetup: IRTokenSetup = {
      assets: [convexAsset.address],
      primaryBasket: [curveStableCoinLPCollateral.address],
      weights: [fp('1')],
      backups: [],
  ...
  describe('Deployment', () => {
    ...
    it('Should setup RToken, Assets, and Collateral correctly', async () => {
      ...
      // Convex Asset
      expect(await convexAsset.isCollateral()).to.equal(false)
      expect(await convexAsset.erc20()).to.equal(curveLpToken.address)
      expect(await convexAsset.erc20()).to.equal(networkConfig[chainId].tokens.COMP)
      expect(await curveLpToken.decimals()).to.equal(18)
      expect(await convexAsset.strictPrice()).to.be.closeTo(fp('58'), fp('0.5')) // Close to $58 USD - June 2022
      expect(await convexAsset.maxTradeVolume()).to.equal(config.rTokenMaxTradeVolume)

      // Check Collateral plugin
      expect(await curveStableCoinLPCollateral.isCollateral()).to.equal(true)
      expect(await curveStableCoinLPCollateral.referenceERC20Decimals()).to.equal(
        await dai.decimals()
      )
      expect(await curveStableCoinLPCollateral.erc20()).to.equal(curveLpToken.address)
      expect(await curveLpToken.decimals()).to.equal(8)
      expect(await curveStableCoinLPCollateral.targetName()).to.equal(
        ethers.utils.formatBytes32String('USD')
      )
      expect(await curveStableCoinLPCollateral.refPerTok()).to.be.closeTo(fp('0.022'), fp('0.001'))
      expect(await curveStableCoinLPCollateral.targetPerRef()).to.equal(fp('1'))
      expect(await curveStableCoinLPCollateral.pricePerTarget()).to.equal(fp('1'))
      expect(await curveStableCoinLPCollateral.prevReferencePrice()).to.equal(
        await curveStableCoinLPCollateral.refPerTok()
      )
      ...
      expect(await curveStableCoinLPCollateral.maxTradeVolume()).to.equal(
        config.rTokenMaxTradeVolume
      )
    ...
    it('Should register ERC20s and Assets/Collateral correctly', async () => {
      ...
      expect(ERC20s[2]).to.equal(convexAsset.address)
      expect(ERC20s[3]).to.equal(dai.address)
      expect(ERC20s.length).to.eql(4)

      // Assets
      expect(await assetRegistry.toAsset(ERC20s[0])).to.equal(rTokenAsset.address)
      expect(await assetRegistry.toAsset(ERC20s[1])).to.equal(rsrAsset.address)
      expect(await assetRegistry.toAsset(ERC20s[2])).to.equal(convexAsset.address)
      expect(await assetRegistry.toAsset(ERC20s[3])).to.equal(curveStableCoinLPCollateral.address)

      // Collaterals
      expect(await assetRegistry.toColl(ERC20s[3])).to.equal(curveStableCoinLPCollateral.address)
    })
    ...
    it('Should register Basket correctly', async () => {
      ...
      expect(backing[0]).to.equal(curveLpToken.address)
