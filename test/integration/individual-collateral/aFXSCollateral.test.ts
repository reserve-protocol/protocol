...
const holderAFXS = '0x640a11d9a4b1db94b30a9cca25dc1e6870d7afc3'

const describeFork = useEnv('FORK') ? describe : describe.skip

describeFork(`aFXSCollateral - Mainnet Forking P${IMPLEMENTATION}`, function () {
  ...
  let aFXSCollateralFactory: ContractFactory
  ...
  before(async () => {
    await setup(forkBlockNumber['concentrator-fxs-pool'])
  ...
  beforeEach(async () => {
    ...
    await whileImpersonating(holderAFXS, async (aFxsSigner) => {
      await aFxs.connect(aFxsSigner).transfer(addr1.address, initialBal)
    })
    ...
    const rTokenSetup: IRTokenSetup = {
      assets: [],
      primaryBasket: [aFxsCollateral.address],
      weights: [fp('1')],
      backups: [],
      beneficiaries: [],
    }
  ...
  describe('Deployment', () => {
    ...
    it('Should setup RToken, Assets, and Collateral correctly', async () => {
      ...
      expect(await aFxsCollateral.isCollateral()).to.equal(true)
      expect(await aFxsCollateral.erc20()).to.equal(aFxs.address)
      expect(await aFxs.decimals()).to.equal(18)
      expect(await aFxsCollateral.targetName()).to.equal(ethers.utils.formatBytes32String('FXS'))
      expect(await aFxsCollateral.refPerTok()).to.be.closeTo(fp('1'), fp('0.01'))
      expect(await aFxsCollateral.targetPerRef()).to.equal(fp('1'))
      expect(await aFxsCollateral.pricePerTarget()).to.equal(fp('1'))
    ...
    it('Should register ERC20s and Assets/Collateral correctly', async () => {
      ...
      expect(ERC20s[2]).to.equal(aFxs.address)
      expect(ERC20s.length).to.eql(3)

      // Assets
      expect(await assetRegistry.toAsset(ERC20s[0])).to.equal(rTokenAsset.address)
      expect(await assetRegistry.toAsset(ERC20s[1])).to.equal(rsrAsset.address)
      expect(await assetRegistry.toAsset(ERC20s[2])).to.equal(aFxsCollateral.address)

      // Collaterals
      expect(await assetRegistry.toColl(ERC20s[2])).to.equal(aFxsCollateral.address)
    })
    ...
    it('Should register Basket correctly', async () => {
      ...
      expect(backing[0]).to.equal(aFxs.address)
      ...
      expect(price).to.be.closeTo(fp('6.55'), fp('0.01'))

      // Check RToken price
      const issueAmount: BigNumber = bn('1000e18')
      await aFxs.connect(addr1).approve(rToken.address, issueAmount)
      await expect(rToken.connect(addr1).issue(issueAmount)).to.emit(rToken, 'Issuance')
      expect(await rTokenAsset.strictPrice()).to.be.closeTo(fp('6.55'), fp('0.01'))
    })
  })
  ...
  describe('Issuance/Appreciation/Redemption', () => {
    it('Should issue, redeem, and handle appreciation rates correctly', async () => {
      ...
      const balanceAddr1aCrv: BigNumber = await aFxs.balanceOf(addr1.address)

      // Check rates and prices
      const aCrvPrice1: BigNumber = await aFxsCollateral.strictPrice()
      const aCrvRefPerTok1: BigNumber = await aFxsCollateral.refPerTok()

      expect(aCrvPrice1).to.be.closeTo(fp('6.55'), fp('0.01'))
      expect(aCrvRefPerTok1).to.be.closeTo(fp('1'), fp('0.01'))
      ...
      //await aFxs.harvest(addr1.address, 0)
      ...
      expect(aCrvPrice2).to.be.closeTo(fp('6.55'), fp('0.01'))
      expect(aCrvRefPerTok2).to.be.closeTo(fp('1'), fp('0.01'))
      ...
      const aCrvPrice3: BigNumber = await aFxsCollateral.strictPrice()
      const aCrvRefPerTok3: BigNumber = await aFxsCollateral.refPerTok()
      ...
      expect(aCrvPrice3).to.be.closeTo(fp('6.56'), fp('0.01'))
      expect(aCrvRefPerTok3).to.be.closeTo(fp('1.001'), fp('0.001'))
      ...
      // Check balances - Fewer aFxs should have been sent to the user
      const newBalanceAddr1aCrv: BigNumber = await aFxs.balanceOf(addr1.address)

      // Check received tokens represent ~1K in value at current prices
      expect(newBalanceAddr1aCrv.sub(balanceAddr1aCrv)).to.be.closeTo(fp('998'), fp('1'))

      // Check remainders in Backing Manager
      expect(await aFxs.balanceOf(backingManager.address)).to.be.closeTo(fp('1.25'), fp('0.01'))
  ...
  describe('Collateral Status', () => {
    it('Updates status in case of hard default', async () => {
      ...
      await aFxsMock.setUnderlying(fp('99'))
