...
describeFork(`Asset Plugins - Integration - Mainnet Forking P${IMPLEMENTATION}`, function () {
  ...
  describe('Assets/Collateral', () => {
    ...
    beforeEach(async () => {
      ...
      // Get plain aTokens
      aDai = <IAToken>(
        await ethers.getContractAt('contracts/plugins/assets/aave/vendor/IAToken.sol:IAToken', networkConfig[chainId].tokens.aDAI || '')
      )

      aUsdc = <IAToken>(
        await ethers.getContractAt('contracts/plugins/assets/aave/vendor/IAToken.sol:IAToken', networkConfig[chainId].tokens.aUSDC || '')
      )
      aUsdt = <IAToken>(
        await ethers.getContractAt('contracts/plugins/assets/aave/vendor/IAToken.sol:IAToken', networkConfig[chainId].tokens.aUSDT || '')
      )
      aBusd = <IAToken>(
        await ethers.getContractAt('contracts/plugins/assets/aave/vendor/IAToken.sol:IAToken', networkConfig[chainId].tokens.aBUSD || '')
      )

      aUsdp = <IAToken>(
        await ethers.getContractAt('contracts/plugins/assets/aave/vendor/IAToken.sol:IAToken', networkConfig[chainId].tokens.aUSDP || '')
      )
      // Get collaterals
    ...
    context('With Complex basket', function () {
      ...
      beforeEach(async () => {
        ...
        await cWBTCVault
          .connect(addr1)
          .approve(rToken.address, await cWBTCVault.balanceOf(addr1.address))
        await weth.connect(addr1).approve(rToken.address, await weth.balanceOf(addr1.address))
        await cETHVault
          .connect(addr1)
          .approve(rToken.address, await cETHVault.balanceOf(addr1.address))
  ...
  describe.skip('Claim Rewards - ATokens/CTokens Fiat', () => {
    ...
    beforeEach(async () => {
      ...
      cDaiVault = <CTokenVault>(
        await ethers.getContractAt('CTokenVault', await cDaiCollateral.erc20())
      )
