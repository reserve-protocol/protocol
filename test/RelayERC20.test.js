...
describe("RelayERC20 contract", function () {
  ...
  describe("Transactions", function () {
    ...
    it("Should update balances after transfers", async function () {
      ...
      expect(finalOwnerBalance).to.equal(initialOwnerBalance - amount1.add(amount2));
  ...
  describe("Relay Transfers", function () {
    it("Should perform relay transfer between accounts", async function () {
      ...
      await expect(token.connect(addr2).relayedTransfer(sig, owner.address, addr1.address, amount, 0))
    ...
    it("Should perform relay transfer of fee", async function () {
      ...
      await expect(token.connect(addr2).relayedTransfer(sig, owner.address, addr1.address, amount, fee))
        .to.emit(token, 'TransferForwarded')
    ...
    it("Should update nonce correctly and only accept valid nonce", async function () {
      ...
      await token.connect(addr2).relayedTransfer(sig, owner.address, addr1.address, amount1, 0);
      ...
      await expect(token.connect(addr2).relayedTransfer(sig2, owner.address, addr1.address, amount2, 0))
        .to.emit(token, 'TransferForwarded')
