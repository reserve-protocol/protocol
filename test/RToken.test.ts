...
describe("RToken contract", function () {
    ...
    describe("Updates/Changes to Basket", function () {
        ...
        it("Should not allow to update basket if not Owner", async function () {
            ...
            // Update basket
            await expect(rToken.connect(addr1).updateBasket(newTokens)).to.be.revertedWith(
                "Ownable: caller is not the owner"
            )
        ...
        it("Should validate slippage tolerance", async function () {
            ...
            // Update basket
            await expect(rToken.connect(owner).updateBasket(newTokens)).to.be.revertedWith(
                "SlippageToleranceTooBig()"
            )
    ...
    describe("Slow Minting", function () {
        ...
        it("Should process multiple Mintings in queue in single issuance", async function () {
            ...
            expect(await rToken.balanceOf(owner.address)).to.equal(
                amount1.add(amount2).add(amount3).add(amount4)
            )
    ...
    describe("Redeem", function () {
        ...
        it("Should redeem RTokens correctly for multiple basket tokens and users", async function () {
            ...
            expect(await bskToken2.balanceOf(rToken.address)).to.equal(
                mintAmount_tkn2.mul(2).sub(redeemAmount_tkn2.mul(2))
            )
            ...
            expect(await bskToken3.balanceOf(rToken.address)).to.equal(
                mintAmount_tkn3.mul(2).sub(redeemAmount_tkn3.mul(2))
            )
    ...
    describe("Rebalancing", function () {
        ...
        it("Should allow to freeze rebalancing", async function () {
            ...
            expect(await rsrToken.balanceOf(addr1.address)).to.equal(
                prevRSRBalanceAddr1.sub(freezeCost)
            )
            expect(await rsrToken.balanceOf(rToken.address)).to.equal(
                prevRSRBalanceRToken.add(freezeCost)
            )
        })
        ...
        it("Should not allow to unfreeze if not frozen", async function () {
            ...
            await expect(rToken.connect(addr1).unfreezeRebalancing()).to.be.revertedWith(
                "RebalancingAlreadyUnfrozen()"
            )
        ...
        context("With frozen rebalancing", async function () {
            ...
            it("Should not allow to unfreeze rebalancing if not freezer", async function () {
                ...
                await expect(rToken.connect(addr2).unfreezeRebalancing()).to.be.revertedWith(
                    "Unauthorized()"
                )
            ...
            it("Should allow freezer to unfreeze rebalancing", async function () {
                ...
                expect(await rToken.freezer()).to.equal(ZERO_ADDRESS)
                expect(await rsrToken.balanceOf(addr1.address)).to.equal(
                    prevRSRBalanceAddr1.add(freezeCost)
                )
                expect(await rsrToken.balanceOf(rToken.address)).to.equal(
                    prevRSRBalanceRToken.sub(freezeCost)
                )
            })
            ...
            it("Should allow to freeze rebalancing even if already frozen", async function () {
                ...
                expect(await rsrToken.balanceOf(addr2.address)).to.equal(
                    prevRSRBalanceAddr2.sub(freezeCost)
                )
