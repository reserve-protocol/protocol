...
// Deploy Curve Volatile Plugin for Tricrypto
...
async function main() {
  ...
  /********  Deploy Curve Volatile Pool for 3pool  **************************/
  ...
  const w3Pool = await CurveStakingWrapperFactory.deploy(
    TRI_CRYPTO_TOKEN,
  ...
  console.log(
    `Deployed wrapper for Curve Volatile TriCrypto on ${hre.network.name} (${chainId}): ${w3Pool.address} `
  )
  ...
  console.log(
    `Deployed Curve Volatile Collateral to ${hre.network.name} (${chainId}): ${collateral.address}`
  )

  assetCollDeployments.collateral.crvTriCrypto = collateral.address
  assetCollDeployments.erc20s.crvTriCrypto = w3Pool.address
