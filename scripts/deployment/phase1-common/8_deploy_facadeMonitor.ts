...
async function main() {
  ...
  console.log(`Deploying Facade Monitor to network ${hre.network.name} (${chainId})
  ...
  // Deploy FacadeMonitor
  const FacadeMonitorFactory = await ethers.getContractFactory('FacadeMonitor')

  facadeMonitor = <FacadeMonitor>await FacadeMonitorFactory.connect(burner).deploy()
  await facadeMonitor.deployed()

  // Write temporary deployments file
  deployments.facadeMonitor = facadeMonitor.address
  fs.writeFileSync(deploymentFilename, JSON.stringify(deployments, null, 2))

  console.log(`Deployed to ${hre.network.name} (${chainId})
    FacadeMonitor:  ${facadeMonitor.address}
