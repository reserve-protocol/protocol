...
async function main() {
  ...
  const w3PoolCollateral = await ethers.getContractAt(
    ...
    deployments.collateral.crv3Pool as string
  )

  /********  Verify CurveGaugeWrapper  **************************/

  await verifyContract(
    chainId,
    await w3PoolCollateral.erc20(),
    [THREE_POOL_TOKEN, 'Wrapped Staked Curve.fi DAI/USDC/USDT', 'ws3Crv', CRV, THREE_POOL_GAUGE],
    'contracts/plugins/assets/curve/crv/CurveGaugeWrapper.sol:CurveGaugeWrapper'
  )

  /********  Verify 3Pool plugin  **************************/
  await verifyContract(
    chainId,
    deployments.collateral.crv3Pool,
    [
      {
