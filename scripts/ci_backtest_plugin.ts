...
export const main = async () => {
  const runTest = async (backtestConfig: string) => {
    console.log("Running backtest " + backtestConfig)
    const provider = new providers.JsonRpcProvider(process.env.MAINNET_RPC_URL)

    const config = JSON.parse(fs.readFileSync("./scripts/backtester/configs/" + backtestConfig + ".json", "utf8")) as IBacktestType
